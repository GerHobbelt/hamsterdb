...
@sa ham_record_t
*/
#define HAM_RECORDSIZE_TOO_SMALL     (-34)
/** User-allocated key size too small

@sa HAM_KEY_USER_ALLOC
@sa ham_key_t
*/
#define HAM_KEYSIZE_TOO_SMALL        (-35)
...
#define HAM_CYCLE_IN_DEVICE_GRAPH    (-38)
/** Key already exists */
#define HAM_KEY_ALREADY_EXISTS       (-39)
/** Backend already initialized */
#define HAM_BACKEND_ALREADY_INITIALIZED         (-40)
...
/**
 * A warning message: warnings are recoverable errors in the sense that hamsterdb will
 * already have corrected for them as best it can.
 *
 * Also, warnings rather than errors
 * are issued for rejected input parameters: after all the 'recovery' in that case
 * is simply not executing the call at all, while @e errors are issues which pop up
 * while performing a job and running into some sort of wall while on the job.
 *
 * Internally, these messages are generated by @ref ham_logwarn() calls.
 */
#define HAM_DEBUG_LEVEL_WARNING     1

/**
 * An error message
 *
 * Internally, these messages are generated by @ref ham_logerr() calls.
 */
#define HAM_DEBUG_LEVEL_ERROR      2
 ...
 *            @ref ham_set_duplicate_compare_func. This flag was not persistent.
 *
 *            <em>WARNING</em>: v2.0.1 and older v2 releases do not allow
 *            this flag in combination with @ref HAM_ENABLE_TRANSACTIONS.
 *
 ...
 *
 * TODO Ger: fix that last paragraph and the corresponding code!
 *
 ...
 * @sa ham_parameter_function_t
 */
#define HAM_PARAM_LOG_DIRECTORY      0x00000107u
 ...
 *              Databases of this Database's Environment
 *        <li>@ref HAM_PARAM_LOG_DIRECTORY The path of the log file
 *            and the journal files
 ...
 * @ref HAM_DIRECT_ACCESS is only allowed in In-Memory Databases and not if
 ...
 *
 * ** TODO ** Ger: IIRC my code does allow partils for tiny records: after all, the user may not know up front that it IS a tiny record!
 *
 ...
 *        is not large enough to store the entire (or partial) record.
 *
 ...
 * partial_size+partial_offset exceed record->size then partial_size should
 * have been limited to the record->size upper bound by the user. To shrink
 * or grow the record, adjust record->size.
 * @ref HAM_PARTIAL automatically overwrites existing records.
 * Gaps will be filled with NUL-bytes if the record did not yet exist or
 * when it is expanded.
 *
 ...
 *        <li>@ref HAM_DIRECT_ACCESS Only for In-Memory Databases and
