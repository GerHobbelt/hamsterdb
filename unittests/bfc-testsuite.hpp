...
protected:
    ...
    virtual void ditch_all_caught_values(void) {
        for (unsigned i=0; i<_caught_values.size(); i++) {
            if (_caught_values[i]) {
                delete _caught_values[i];
                _caught_values[i]=0;
            }
        }
        //_caught_values.resize(0); -- removed to increase performance
    }
