...
public:
    ...
    void structureTest(void)
    {
        ...
        char *p=(char *)&key_get_key(key)[0];
        *p='a';
        BFC_ASSERT_EQUAL('a', (char)(*key_get_key(key)));
            ...
            ham_u32_t flags)
    {
        ...
        if (!(flags&HAM_DUPLICATE)) {
            ...
            BFC_ASSERT_EQUAL(0, util_read_record(m_db, &rec2, &rec2._rid, 0));
            ...
            const char *data, ham_size_t size)
    {
        ...
        BFC_ASSERT_EQUAL(0, util_read_record(m_db, &rec, &rec._rid, 0));
